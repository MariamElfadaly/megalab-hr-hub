...
import { getQrDataUrl, printLabels, escapeHtml } from "../lib/labelUtils";
...
export default function LabelGenerator() {
  ...
  function printRegistryReport() {
    const rows = roster.map((e) => {
      const has = !!recordsById[e.id]?.qrLink;
      return `<tr><td>#${escapeHtml(e.id)} — ${escapeHtml(e.nameEn)}</td><td>${escapeHtml(e.titleEn || "—")}</td><td>${has ? "Yes" : "No"}</td></tr>`;
    }).join("");
    const withLink = roster.filter((e) => !!recordsById[e.id]?.qrLink).length;

    document.getElementById("printArea").innerHTML = `
      <div class="rp-brand">MegaLab · Label Generator</div>
      <h1>Label Registry — All Employees</h1>
      <p style="font-size:12px;color:#555;margin-bottom:12px;">Printed ${new Date().toLocaleDateString()} · ${withLink}/${roster.length} have a Drive link on file</p>
      <table>
        <tr><th>Employee</th><th>Title</th><th>Has Drive link</th></tr>
        ${rows}
      </table>
    `;
    window.print();
  ...
  function printEmployeeReport(emp) {
    ...
    document.getElementById("printArea").innerHTML = `
      <div class="rp-brand">MegaLab · Label Generator</div>
      <h1>Employee Label Report</h1>
      <table>
        <tr><th>Name</th><td>${escapeHtml(emp.nameEn)}</td><th>ID</th><td>#${escapeHtml(emp.id)}</td></tr>
        <tr><th>Name (Arabic)</th><td>${escapeHtml(emp.nameAr)}</td><th>Title</th><td>${escapeHtml(emp.titleEn)} ${emp.titleAr ? "/ " + escapeHtml(emp.titleAr) : ""}</td></tr>
        <tr><th>Drive link</th><td colspan="3">${link ? escapeHtml(link) : "Not set"}</td></tr>
      </table>
      <div class="rp-footer"><span>Printed ${new Date().toLocaleDateString()}</span><span>MegaLab Label Generator</span></div>
    `;
    window.print();
  }
    ...
    <div className="lg-layout">
      <div className="lg-registry">
        <div className="lg-toolbar">
          ...
          <button onClick={printRegistryReport} disabled={roster.length === 0}>
      ...
      <div className="lg-editor">
        {!selected ? (
          ...
          <>
            ...
            <button className="lg-printReportBtn" onClick={() => printEmployeeReport(selected)}>
